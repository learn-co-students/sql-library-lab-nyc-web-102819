...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters ON series.author_id = characters.author_id GROUP BY series.title HAVING characters.species = 'human' AND count(characters.name) ORDER BY count(*) DESC LIMIT 1"
end
